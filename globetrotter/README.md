...
# Seed the database with 20+ Indian destinations, landmark cover photos & agency feeds
npm run seed
...
3. Select the `frontend` folder, set build command to `npm run build` and output directory to `dist`.
4. Deploy!

### Deploy Backend to Render
1. Log in to [Render](https://render.com) and create a **Web Service**.
2. Select the `backend` folder, set build command to `npm install` and start command to `node server.js`.
3. Deploy!
